...
# accounts). The syntax ${string//-/} in a few of these commands does a
...
az group create --name $SCENARIO_RG --location $AZURE_LOCATION
...
az storage account create --name $THIRD_PARTY_API_STORAGE_NAME --location $AZURE_LOCATION --resource-group $SCENARIO_RG --sku Standard_LRS
...
az webapp up --name $MAIN_APP_NAME --plan $MAIN_APP_PLAN_NAME --sku B1 --resource-group $SCENARIO_RG --location $AZURE_LOCATION
...
echo "Retrieving the main app object ID"
...
az storage account create --name $MAIN_APP_STORAGE_NAME --location $AZURE_LOCATION --resource-group $SCENARIO_RG --sku Standard_LRS
...
az storage queue create --name $STORAGE_QUEUE_NAME --account-name $MAIN_APP_STORAGE_NAME --connection-string $MAIN_APP_STORAGE_CONN_STRING
...
az role assignment create --assignee $MAIN_APP_OBJECT_ID --role "Storage Queue Data Contributor" --scope "/subscriptions/$AZURE_SUBSCRIPTION_ID/resourceGroups/$SCENARIO_RG/providers/Microsoft.Storage/storageAccounts/$MAIN_APP_STORAGE_NAME/queueServices/default/queues/$STORAGE_QUEUE_NAME"
...
az keyvault create --name $KEY_VAULT_NAME --resource-group $SCENARIO_RG --location $AZURE_LOCATION

echo "Setting secret $THIRD_PARTY_API_SECRET_NAME to value $THIRD_PARTY_API_SECRET_VALUE"

az keyvault secret set --vault-name $KEY_VAULT_NAME --name $THIRD_PARTY_API_SECRET_NAME --value $THIRD_PARTY_API_SECRET_VALUE
...
az keyvault set-policy --name $KEY_VAULT_NAME --object-id $MAIN_APP_OBJECT_ID --resource-group $SCENARIO_RG --secret-permissions get
...
#
# You can now use the test.sh script to invoke the API endpoint and
# then retrieve the queue message that the endpoint generated.

echo "Press any key to test the deployment by running test.sh."
